<!-- Container content -->
<div class="container__content">
    <div class="inner_content">
        <div class="container_heading">
            <h2 class="heading_content">Current Series</h2>
        </div>
        <div class="container_series">
            <div class="cardSeries">
                <a href="/">
                    <div class="imgWrap">
                        <img class="img_series" src="https://imgs.search.brave.com/aUNyvZBXUulb963JH7KnQm9AMr8bcBoLsiHREOqayIU/rs:fit:612:612:1/g:ce/aHR0cHM6Ly9pNS53/YWxtYXJ0aW1hZ2Vz/LmNvbS9hc3IvOWZm/ZWYzMDMtMGZhYy00/OGRkLTg3ODctYzUy/NTk0MDk2ODAwXzEu/MTc1ZDk1Mjg2NzY0/OGEwOTczMTY2NGMy/MTE1NjNlYWIuanBl/Zz9vZG5XaWR0aD02/MTImb2RuSGVpZ2h0/PTYxMiZvZG5CZz1m/ZmZmZmY" alt="">
                    </div>
                    <div class="infoWrap">
                        <p class="info_content title_series">Title</p>
                        <p class="info_content price_series">Price</p>
                        <p class="info_content sale_series">Sale Date</p>
                    </div>
                </a>
            </div>
        </div>
        <div class="container_cta">
            <button type="button" class="btn_load_more inverted_btn">Load More</button>
        </div>
    </div>
</div>